...
// signer, only that a well-formed line exists to verify the new signature
// against) and a `! pigpen-v1` type line, with no self-signature line yet.
...
// carrying a fresh papi-pigpen-self-sig-v1@ecdsa_p256_sig `-` line. A second
...
func TestPigpenSignCmd(t *testing.T) {
	...
	if !strings.Contains(out.String(), "papi-pigpen-self-sig-v1@ecdsa_p256_sig-") {
		t.Errorf("signed output missing a papi-pigpen-self-sig-v1@ecdsa_p256_sig line:\n%s", out.String())
	}
