...
import (
	...
	"code.linenisgreat.com/papi/internal/bravo/authproxy"
...
func main() {
	...
	root.AddCommand(newGenerateManCmd())
...
// newGenerateManCmd is a hidden build-time helper that renders the cobra command
// tree into section-1 man pages under <dir>/share/man/man1 (eng-manpages(7) Go
// codegen). The flake's postInstall runs `papi generate-man $out`, so `nix build`
// ships papi(1) plus one page per subcommand alongside the binary; the pages stay
// in lockstep with --help rather than drifting like hand-written command docs.
// Hand-written section-7 concept pages (papi-ssh-sync, papi-bootstrap) are
// compiled from doc/*.7.scd by scdoc, not here.
//
// It lives inside `package main` because that is where the command tree is
// assembled; the eventual pivot (papi#66) extracts the CLI into an importable,
// dagnabit-structured package and replaces this with a standalone cmd/papi-gen_man
// binary (eng-manpages(7) "separate gen_man binary").
func newGenerateManCmd() *cobra.Command {
	return &cobra.Command{
		Use:    "generate-man <dir>",
		Short:  "Render section-1 man pages into <dir>/share/man/man1 (build helper)",
		Hidden: true,
		Args:   cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			man1 := filepath.Join(args[0], "share", "man", "man1")
			if err := os.MkdirAll(man1, 0o755); err != nil {
				return err
			}
			root := cmd.Root()
			// Suppress cobra's dated "Auto generated by ..." footer so the
			// pages are byte-reproducible across builds (Nix purity).
			...
			return doc.GenManTree(root, header, man1)
		},
	}
}
...
func newPigpenSignCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Read an unsigned (or not-yet-self-signed) hyphence /papi/pigpen document on stdin, " +
			...
			"carries no `-`-prefixed piggy-piv_auth-v1@ssh_ecdsa_nistp256_pub key line to verify " +
			"the new signature against. Unlike sign-challenge this signature is document-bound, not domain-bound: " +
